  ...
  return {
    ...
    buildStart() {
      ...
      server.listen(NOTIFY_PORT, () => {
        console.log(`[metabase-notify] SSE server listening on http://localhost:${NOTIFY_PORT}`);
      });
    },
    ...
    closeBundle() {
      ...
      console.log(`[metabase-notify] Build complete, notified ${clients.size} client(s)`);
    },
  };
}
